...
description: "Analyze ETL source files (CSV, Excel) for encoding, delimiters, date formats, and cleaning rules. Trigger: ETL source file analysis."
license: Apache-2.0
metadata:
...
header authorship rule (§2) when emitting a study-file ETL probe, the 4-pattern
taxonomy (§3) for relating detected encoding/delimiter quirks back to the ETL
pattern, and master-project awareness (§4) — source files live in the CARGA
repo, study output is recorded next to the job.
