...
description: "Guide the three-phase workflow for creating new Glue tables: schema extraction, INFRA table definition, enable writes. Trigger: new Glue table creation."
license: Apache-2.0
metadata:
...
This skill follows the shared data-engineer protocol. Relevant sections:
master-project awareness (§4) — the three phases deliberately cross the
master (INFRA) and carga (CARGA) repos in sequence; AWS profile logical-name
rule (§5) — every CLI invocation uses `ResolveProfile(cfg, "dev")`, never a
hardcoded profile; and the header authorship rule (§2) on the Phase 1 ETL
probe.
...
The workflow crosses both repos. Phase boundaries are commit boundaries — each
phase lands on the repo it touches so review can bisect cleanly.
...
The same structure is captured by the sidecar schema file emitted by
`sdd-spec` (`glue-tables/{db}.{table}.yaml`) and validated by
`internal/etl.ValidateSidecar` against the live `aws glue get-table` output
during `sdd-verify` Camino B. Keep the YAML and the sidecar in lockstep.
...
All AWS CLI invocations resolve the profile through `ResolveProfile(cfg, "dev")`
(see protocol §5). Never paste the concrete profile string into a command or
a comment.
...
aws cloudformation deploy \
    ...
    --profile "$(gentle-ai sdd-config --json | jq -r .aws_profiles.dev)"

# Verify table created
aws glue get-table \
    --database-name toyota_chile_dev \
    --table-name stg_table_name \
    --profile "$(gentle-ai sdd-config --json | jq -r .aws_profiles.dev)"
```
...
Any output that leaves the engineer's machine (PR description, verify report,
issue comment) MUST pass through `ScrubProfiles(text, cfg)` to strip both the
profile name and any bare 12-digit account ID.
