...
> If `domain == "data-engineering"`, follow THIS section (Camino A) AND the
...
> exactly as before (app-dev behavior is untouched). Never infer the domain
> from the prompt.

When the gate is active, implementation follows **Camino A** (the Glue TDD
loop) instead of the app-dev `go test` loop. The contract of a Glue job is the
table it materializes, so the TDD loop proves table-level parity, not function
return values. Every `.py` file you generate or modify MUST pass
`internal/etl.ValidateHeader`, and modifications MUST preserve the original
human author via `internal/etl.UpdateHeaderForModify`.

### Camino A — Glue TDD loop

1. **Glue Docker environment.** Stand up the local Glue runtime so the job
   runs byte-identically to production:
   ```bash
   docker run -it --rm \
     -v "$PWD":/home/glue_user/workspace \
     -e AWS_PROFILE=$(gentle-ai sdd-config --json | jq -r '.aws_profiles.dev // empty') \
     public.ecr.aws/glue/aws-glue-libs:5 spark-shell
   ```
   Resolve the `dev` profile through `gentle-ai sdd-config --json` — never
   hard-code a profile name or account ID into a command, artifact, or log.

2. **Throwaway `_test` table.** Create a disposable target table
   `<db>.<table>_test` in the `dev` database so the TDD loop writes nowhere
   near production. Drop it at the end of the cycle.

3. **TDD scaffold (selected by `DetectPattern`).** Read the pattern from
   `internal/etl.DetectPattern` (or `sdd-config`/design which already
   recorded it) and pick the matching scaffold. This is the TDD loop's
   shape — do NOT invent a different one:
   - `incremental` → **watermark test**: seed a watermark window, run, assert
     only rows in `[lower, upper]` land in `<table>_test`; re-run with a
     frozen window and assert zero new rows (idempotency).
   - `multi-step` → **multi-step DAG test**: materialize each temp view in
     order; assert each stage's rows before the target; the insertion-point
     stages from design are the regression baseline (byte-identical
     pre/post).
   - `glue-studio` → **Athena-Spark parity test**: run the job in the Docker
     Spark runtime AND query the same logic via Athena; assert
     `<table>_test` EXCEPT Athena result is empty.
   - `legacy-wrangler` → migrate off awswrangler.athena first; the TDD
     scaffold is the multi-step DAG test once the GlueContext migration
     lands.
   - unknown pattern → STOP and report `blocked: pattern-undetected`; surface
     the partial-signal rationale from `DetectPattern` and ask the user to
     confirm the pattern before scaffolding.

4. **RED → GREEN → REFACTOR over the table.** Write the EXCEPT/comparison
   test against `<table>_test` first (RED), implement the `.py` job to make
   it pass (GREEN), refactor with the table test still green. The comparison
   query is built with `internal/etl.BuildExceptSQL(target, devDB, prdDB)`.

5. **Header enforcement (every `.py`).** Each generated or modified ETL file
   MUST carry the six-field header from `internal/etl.ETLHeader`, rendered by
   `internal/etl.RenderHeader`:
   ```
   # Glue job           : <job name>
   # desarrollado por   : <original developer>
   # fecha creación     : <YYYY-MM-DD>
   # modificado por     : <modifier>
   # fecha modificación : <YYYY-MM-DD>
   # descripción        : <description>
   ```
   - **New file**: `desarrollado por` and `modificado por` are BOTH the
     directing human; `fecha creación` and `fecha modificación` are BOTH
     today.
   - **Modified file**: call `internal/etl.UpdateHeaderForModify(orig, human,
     today)` — it preserves `desarrollado por` + `fecha creación` and updates
     only `modificado por` + `fecha modificación`.
   - **Validate**: before writing, run `internal/etl.ValidateHeader(content)`
     on the full file. It MUST return nil. It rejects `Co-Authored-By`,
     `generated by`, and AI model/vendor names (Claude, GPT, ChatGPT, Gemini,
     Copilot, OpenAI, Anthropic). AI-directed code attributes authorship to
     the directing human — never to an AI.

6. **Persist progress.** Mark tasks `[x]` as usual and record the Camino A
   evidence (pattern chosen, scaffold, EXCEPT result) in apply-progress. All
   report text runs through `internal/sddconfig.ScrubProfiles` so no profile
   name or account ID leaks.
...
Parse `.domain`. If `data-engineering`, Camino A (Glue TDD loop) is MANDATORY
and the header-enforcement + `UpdateHeaderForModify` authorship rules apply to
every `.py`. If absent, skip Camino A and proceed with the app-dev flow below
— the rest of this skill is identical to its prior behavior.
...
"data-engineering"`, Camino A is required (Glue Docker `aws-glue-libs:5` TDD
loop + throwaway `_test` table + header enforcement via
`internal/etl.ValidateHeader`/`UpdateHeaderForModify`). That loop exceeds this
small-model read budget — STOP and return `needs-explore` so a capable model
runs Camino A. App-dev work (domain absent) continues with the Steps below.
